...
# === Parameters ===
DATA_DIR = "Data"
FILENAME = "L_dynamique3y200dis2_0113.csv"
...
os.makedirs(os.path.join("Results", "Experiment Transformed"), exist_ok=True)
os.makedirs(os.path.join("Results", "velocity_frames"), exist_ok=True)
...
def plot_3d_snapshot(P, v_orig, v_cor, t):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join("Results", "Experiment Transformed", f"snapshot_{t}.png"), dpi=300)
...
def plot_multiple_snapshots(points, v_orig, v_cor, steps):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join("Results", "Experiment Transformed", "snapshots.png"), dpi=300)


def animate_velocity(points, v_orig, v_cor, figname="velocity_animation.gif", filename=FILENAME,  fps=10):
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')
    ...
    def update(t):
        ...
        if t in save_frames:
            save_path = os.path.join("Results", "velocity_frames", f"frame_{t:04d} for {filename}.png")
    ...
    # Save as GIF
    anim.save(figname, writer=PillowWriter(fps=fps), dpi=100)
    print(f"Animation saved as: {figname}")
    plt.close(fig)
...
animate_velocity(corr_pts, rob_speed, rob_cor_speed, 
                figname=os.path.join("Results", "velocity_frames", f"velocity_animation for {FILENAME}.gif"), filename=FILENAME, fps=10)
